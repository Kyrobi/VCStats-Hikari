...
@staticmethod
async def save_tracking_stats_single(user_id1: int, guild_id1: int):
...
@staticmethod
async def get_user_time_and_leaderboard_position(user_id: int, guild_id: int) -> tuple[int, Optional[int]]:
    return await db_handler.get_user_time_and_position(user_id, guild_id)

# Assuming you have similar helper functions in Python
def milliseconds_to_timestamp(seconds: int) -> str:
